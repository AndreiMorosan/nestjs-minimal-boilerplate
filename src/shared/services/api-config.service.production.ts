...
// A simple parser that supports durations like "30s", "5m", "2h", "1d"
function simpleParseDuration(input: string): number | undefined {
  const matches = input.trim().match(/^(\d+)([smhd])$/i);
  if (!matches) {
    return undefined;
  }
...
export class ApiConfigService {
  ...
  private getDuration(key: string): number {
    ...
    const duration = simpleParseDuration(value);
